 ...
 */
// 参考 Fold Craft Launcher （https://github.com/FCL-Team/FoldCraftLauncher/blob/main/FCL/src/main/java/com/mio/controlconverter/CcUtils.kt）
...
object CcUtils {
    ...
    fun zlShapeToFclRadius(shape: JsonObject?): Long {
        ...
        return Math.max(0, Math.min(1000, radius))
    }
    ...
    fun zlColorToFcl(color: JsonElement?, fallback: Long, alpha: JsonElement? = null): Long {
        ...
        if (c != null) {
            val high = (c ushr 32) and 0xFFFFFFFFL
            val argb = if (high != 0L) high else (c and 0xFFFFFFFFL)
            if (argb != 0L || c == 0L) {
